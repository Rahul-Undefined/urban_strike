...
- test.js auto-gates legacy phases 11/12/14 behind the bot switch (5 SKIP
  notes when off — the shipped default).
...
- **Frag two bands** (`THROWS.frag.killRadius/outerDmg/selfRadius/fxRadius`;
  `weapons/system.js explosionDamage` + `bots.js resolveNades` with server LOS).
...
11. **Area weapons are server modules with a tick and a teardown** (mines,
    ...
    launch; read `Bots.buildColliders` for "can the fire see him" / "is he
    under a roof" — never a client claim.
