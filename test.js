...
function phase15() {
  ...
  function go() {
    ...
    A.emit('createRoom', { name: 'Cfg', settings: { mode: 'co2', map: 'riverside', minutes: 60, killTarget: 0 } }, (res) => {
...
function phase18() {
  ...
  function step1() {
    ...
    A.emit('useEmp', {}, (r) => {
      ...
      {
        /* v2.0: the strike remote is gone — no strike handler answers, no remote is planted */
        const remotes = pickups.filter(e => e.t === 'remote');
        ok(remotes.length === 0, 'v2.0: no strike remote is planted [' + remotes.length + ']');
        ok(!pickups.some(e => e.t === 'strike_key'), 'v2.0: no strike key is planted');
        ok(!pickups.some(e => (CFG.LOOT_ITEMS[e.t] || {}).kind === 'weapon'), 'v2.0: no weapon spawns on the floor — all ' + pickups.length + ' floor pickups are heals/ammo/armour/gear');
  ...
  function stepRemote(remotes) { finishPhase(); }   /* v2.0: the remote is gone */
...
function phase19() {
  ...
  /* v2.0: the rocket ladder phase is gone with the rocket. */
  ...
  /* --- v1.0d: the arena redeploy ladder --- */
  {
    ...
    ok(CFG.respawnDelayFor('urban', 20) === CFG.MATCH.respawnDelay && CFG.respawnDelayFor('riverside', 20) === CFG.MATCH.respawnDelay,
...
function phase1(done) {
  ...
  A.on('matchStart', (d) => {
    ...
    /* v2.0: NO weapon spawns on the floor (every weapon is crate-only) and no
       point is forced; the floor is heals / ammo / armour / gear, ~245 points,
       all interior or roof. */
    ok(!lootList.some(e => CFG.LOOT_ITEMS[e.t].kind === 'weapon'), 'v2.0: no weapon on the floor — weapons come only from crates');
    ok(lootList.every(e => !CFG.LOOT_ITEMS[e.t].drop && !CFG.LOOT_ITEMS[e.t].retired), 'v2.0: nothing crate-only or retired rolled on the floor');
    ok(lootList.length >= 120 && lootList.length <= CFG.LOOT_POINTS.length, 'v2.0: the floor is sparse but present [' + lootList.length + ' of ' + CFG.LOOT_POINTS.length + ' points rolled]');
  });
  B.on('spawn', (d) => {
    if (d.id !== B.id) return;
    bSpawns++;
...
function phase3(done) {
  ...
  B.on('lootAdd', (d) => {
    ...
    /* v2.0: no guaranteed filler — `count` distinct draws from exoticPool only */
    const want = CFG.AIRDROP.count || 5;
    ok(Array.isArray(items) && items.length === want,
      'crate lands with ' + want + ' loot items [' + (items || []).length + ']');
    ok(items.every(e => CFG.AIRDROP.exoticPool.indexOf(e.t) >= 0), 'v2.0: every crate item is from the exotic pool');
    ok(new Set(items.map(e => e.t)).size === items.length, 'v2.0: no two of one item in a crate');
    ok(items.every(e => CFG.LOOT_ITEMS[e.t].drop || CFG.LOOT_ITEMS[e.t].rar === 'l'), 'v2.0: a crate holds only crate-only or legendary things');
    ...
    ok(minGap > 0.5, 'crate items are spread far enough to pick up individually [' +
      minGap.toFixed(2) + 'm]');
    /* v2.0: the crate is exotic-only with no guaranteed kinds; collect the two
       first items, whatever they are, and check each grant matches its kind */
    const first = items[0], second = items[1];
    bPos = [first.p[0], 0.95, first.p[2]];
    setTimeout(() => {
      bPos = [second.p[0], 0.95, second.p[2]];
      setTimeout(() => {
        const kindOf = e => CFG.LOOT_ITEMS[e.t].kind;
        [first, second].forEach(e => {
          const k = kindOf(e);
          /* armour (and helmets) apply to the player record without a grant event; a full-health heal is skipped */
          if (k === 'armor' || k === 'helm' || k === 'heal') return ok(true, 'crate item ' + e.t + ' (' + k + ') applies silently — no grant event by design');
          const want = k === 'weapon' ? ['weapon', 'ammoFor'] : [k];
...
function phase6(done) {
  ...
  const A = io(URL), B = io(URL);
  A.on('connect', () => {
    A.emit('createRoom', { name: 'Am', settings: { map: 'riverside' } }, (res) => {
      B.once('lobby', (lb) => {
        ok(lb.settings && lb.settings.map === 'riverside',
  ...
  B.on('matchStart', (d) => {
    ...
    ok(d.settings && d.settings.map === 'riverside', 'matchStart payload names the map');
  });
  B.on('spawn', (d) => {
    if (d.id !== B.id) return;
    const S = CFG.MAPS_RIVERSIDE.SPAWNS;   /* v2.0: riverside stands in for the removed metro */
    const near = S.some(s => Math.abs(s[0] - d.pos[0]) < 0.6 && Math.abs(s[1] - d.pos[2]) < 0.6);
    ok(near, 'spawn position comes from the RIVERSIDE spawn set [got ' + d.pos[0] + ',' + d.pos[2] + ']');
    [A, B].forEach(s => s.disconnect());
    setTimeout(done, 300);
  });
}
