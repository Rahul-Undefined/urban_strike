...
module.exports = function initHeli(ctx) {
  ...
  function newMachine(idx, state, t) {
    return { idx, pad: (CFG.HELI.pads && CFG.HELI.pads[idx]) || CFG.HELI.pad, state, t0: t, hp: CFG.HELI.hp, riders: [], lastHitBy: null, lastHitAt: 0, boardSince: 0, respawnAt: 0,
      seed: 1 + Math.floor(Math.random() * 2147483000), from: null, refuelUntil: 0, fuelStart: 0 };
  }
  function start(room) {
  ...
  function snapOne(h) { return { idx: h.idx, pad: h.pad, state: h.state, t0: h.t0, hp: h.hp, riders: h.riders.slice(), respawnAt: h.respawnAt || 0, seed: h.seed || 1, from: h.from || null, refuelUntil: h.refuelUntil || 0, fuelStart: h.fuelStart || 0 }; }
  ...
  function bail(room, p) {
    ...
    if (!h || (h.state !== 'flying' && h.state !== 'returning')) return { ok: false, err: 'Not riding' };
    const pose = poseOf(room, h);
  ...
  function tickMachine(room, h, t) {
    ...
    if (h.state === 'pad' || h.state === 'landed') {
      ...
      if (fuelled && h.riders.length && h.boardSince && t - h.boardSince >= CFG.HELI.boardSec * 1000) {
        h.state = 'flying'; h.t0 = t; h.boardSince = 0; h.flightStart = t; h.fuelStart = t; h.emptySince = 0; h.from = null; h.trail = []; h.outCount = {}; h.refuelUntil = 0;
    ...
    if (h.state === 'flying') {
      ...
      /* v1.0x: FUEL — after fuelSec the machine turns for home on its own */
      if (h.fuelStart && t - h.fuelStart >= (CFG.HELI.fuelSec || 180) * 1000) { io.to(room.code).emit('toast', { msg: 'Helicopter ' + (h.idx === 0 ? 'A' : 'B') + ' is out of fuel \u2014 returning to the pad' }); goHome(room, h, pose); return; }
  ...
  function hit(room, shooter, w, idx) {
    ...
    if (!h) { const nr = nearestMachine(room, shooter.pos, ['pad', 'landed', 'flying', 'returning']); if (!nr) return { ok: false, err: 'No helicopter' }; h = nr.m; }
