...
function makeHeli(IDX) {
  ...
  function updateSign() {
    ...
    else if (state.state === 'crash') { text = 'GOING DOWN'; sub = 'out of fuel'; col = '#ff5a4a'; }
  ...
  function inAir() { return !!(state && (state.state === 'flying' || state.state === 'returning' || state.state === 'crash')); }   /* v2.0: a falling cabin is still airborne for its riders */
  ...
  function computePose() {
    ...
    if (state.state === 'crash' && state.crashFrom) return CFG.heliCrashPose(cfg, state.crashFrom, (serverNow() - state.t0) / 1000);   /* v2.0 */
    var q = P.at(0);
  ...
  function update(dt) {
    ...
    /* v2.0: the crash — the machine rolls over as it falls (pose.roll from
       heliCrashPose), a black smoke trail and a fire once it is a wreck */
    if (state.state === 'crash') { targetRoll = pose.roll || 0; targetPitch = 0.35; crashFx(dt); }
    ...
    var spin = state.state === 'flying' ? 0.55 : state.state === 'crash' ? 0.22 : (state.state === 'pad' && state.boarding ? 0.25 : 0.06);
    rotor.rotation.y += spin; tailRotor.rotation.x += spin * 1.6;
    riderHud(); rotorSound(); updateSign(); updatePrompt();
  }
  /* v2.0: crash effects — a smoke puff every 120 ms while falling; the FX
     module ages them out, so nothing here accumulates */
  var crashSmokeAt = 0;
  function crashFx(dt) {
    var t = performance.now();
    if (t - crashSmokeAt < 120 || !pose || typeof FX === 'undefined' || !FX.smokeCloud) return;
    crashSmokeAt = t;
    FX.smokeCloud(new THREE.Vector3(pose.x, pose.y + 1.2, pose.z), 1.6, 1);
  }
  /* v2.0: the low-fuel siren (heliFuel from the server) — loud for riders,
     distant for everyone else; the sign and HUD read the same fuel clock */
  function onFuel(d) {
    if (!d || d.idx !== IDX) return;
    var me = typeof Net !== 'undefined' ? Net.getMyId() : null;
    var aboard = !!(d.riders && me && d.riders.indexOf(me) >= 0);
    if (typeof AudioSys !== 'undefined' && AudioSys.fuelSiren) AudioSys.fuelSiren(aboard);
    if (typeof UI !== 'undefined') {
      if (aboard && UI.setHeliHint) UI.setHeliHint('LOW FUEL \u00b7 ' + (d.left | 0) + ' s \u00b7 Q lands NOW or you go down with it');
      else if (UI.toast) UI.toast('Helicopter ' + (IDX === 0 ? 'A' : 'B') + ' is low on fuel');
    }
  }
  ...
  return { idx: IDX, init: init, dispose: dispose, set: set, clear: clear, hpUpdate: hpUpdate, update: update, floorAt: floorAt, bail: bail, rayHit: rayHit,
    canBoard: canBoard, board: board, onSeat: onSeat, landRequest: landRequest, seatFor: seatFor, isRiderId: isRiderId, nearForBoarding: nearForBoarding, onFuel: onFuel,
...
var Heli = (function () {
  ...
  return {
    ...
    onFuel: function (d) { each(function (m) { m.onFuel(d); }); },   /* v2.0 */
