...
var Train = (function () {
  ...
  function floorAt(pos, halfY) {
    ...
    for (var i = 0; i < cars.length; i++) {
      ...
      else if (c.coach && Math.abs(lz) <= HALF_W - 0.2 && Math.abs(lx) <= c.L / 2 - 0.15 && feet >= FLOOR - 0.95 && feet <= FLOOR + 0.9) res = { y: FLOOR, inside: true, doorZone: doorZone };
      else if (c.coach && doorZone && Math.abs(lz) > HALF_W - 0.2 && feet >= STEP_Y - 0.95 && feet <= FLOOR + 0.6) res = { y: STEP_Y, step: true, doorZone: true };
      else if (feet < FLOOR - 0.2 && Math.abs(lx) <= c.L / 2 + 0.05 && Math.abs(lz) <= HALF_W + 0.05 && !(doorZone && Math.abs(lz) > HALF_W - 0.6)) res = { block: true };
      if (!res) continue;
