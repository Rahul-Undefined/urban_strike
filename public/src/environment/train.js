...
function makeTrain(cfgIn, sceneIn) {
  ...
  var rumbleAt = 0, wasMoving = false;
  function trainSound() {
    if (typeof AudioSys === 'undefined' || typeof PlayerCtl === 'undefined' || !cars.length) return;
    var moving = headV > 0.3;
    if (moving && !wasMoving && AudioSys.trainHorn) AudioSys.trainHorn(new THREE.Vector3(cars[0].x, 3, cars[0].z));   // pulling out
    wasMoving = moving;
    if (!moving || !AudioSys.trainRumble) return;
    var t = performance.now();
    if (t - rumbleAt < 700) return;
    rumbleAt = t;
    var best = null, bd = 1e9;
    for (var i = 0; i < cars.length; i++) { var d = Math.hypot(cars[i].x - PlayerCtl.pos.x, cars[i].z - PlayerCtl.pos.z); if (d < bd) { bd = d; best = cars[i]; } }
    if (best && bd < 60) AudioSys.trainRumble(new THREE.Vector3(best.x, 1.5, best.z), headV / (cfg.speed || 8));
  }
  function update(dt) {
    if (!active) return;
    ...
    for (var i = 0; i < cars.length; i++) {
      ...
      c.carried = false;   /* v2.0: this pose's carry has not been consumed yet (see floorAt) */
  ...
  function floorAt(pos, halfY) {
    ...
    for (var i = 0; i < cars.length; i++) {
      ...
      /* v2.0: (1) the rigid carry is consumed ONCE per train pose — a second
         floorAt in the same frame (or a controller running ahead of the train)
         sees the current pose and no delta, so nobody is pushed twice; (2) the
         stale-pose limit is 9 m, not 4: at 30 m/s a 250 ms frame hitch moves a
         coach 7.5 m, and a rider who is not carried through a hitch is a rider
         on the road behind a train that just left — fatal by the v1.0g rule. */
      var fresh = !c.carried && Math.abs(c.x - c.px) + Math.abs(c.z - c.pz) < 9 && Math.abs(dyaw) < 0.9;
      ...
      if (Math.abs(lx) > c.L / 2 + 0.35 || Math.abs(lz) > HALF_W + 0.65) continue;
      var res = null;
      ...
      if (feet >= ROOF - 0.75 && inBodyXZ) res = { y: ROOF, roof: true };
        ...
        else if (inBodyXZ) res = { block: true, inward: false, high: feet >= FLOOR - 0.5 };   /* v2.0: `high` = at deck height */
        /* v2.0: to someone at DECK height the step-board strip (HALF_W..+0.6)
           is a wall too, except at the doors — so the 0.7 m between a halt's
           deck edge and the coach body cannot be walked into and fallen
           through; the controller pushes them back onto the deck (`high`). */
        else if (!doorZone && feet >= FLOOR - 0.5 && Math.abs(lz) <= HALF_W + 0.6 && Math.abs(lx) <= c.L / 2 - 0.3) res = { block: true, inward: false, high: true };
      }
      ...
      if (!res) continue;
      if (!res.block) c.carried = true;   /* v2.0: a carry result spends this pose's delta */
