  ...
  function update(dt, input, weaponSpeedMult, isAiming) {
    ...
    if (plat && plat.block) {
      /* v1.0f/v1.0i: the train is a WALL to anyone not aboard. `inward` means
         ...
         jumped at the wall): back inside. Otherwise: out to the nearest face. */
      var nlx = plat.lx, nlz = plat.lz;
      /* v2.0: a player at DECK height (plat.high — feet near the coach floor,
         i.e. standing on a halt platform) is pushed to 2.35 m, back onto the
         deck (its edge is 2.2 m from the centreline); at road height the old
         1.9 m stands. Without this a rider pressed into the wall from the
         platform was set down in the 15 cm gap and fell to the road. */
      var side = plat.halfW + 0.35 + (plat.high ? 0.75 : 0.3);
      var toSide = side - Math.abs(plat.lz), toEnd = (plat.halfL + 0.35 + 0.65) - Math.abs(plat.lx);
      if (toSide <= toEnd) nlz = Math.sign(plat.lz || 1) * side;
      ...
      /* v1.0i: EVERYTHING inside the body is carried and walled, standing or
         airborne — a jump inside the coach used to fly through the side. The
         floor snap alone still waits for the feet to be on it. */
      ...
      if (plat.inside) {
        ...
        if (plat.ceil && pos.y + halfY > plat.ceil) { pos.y = plat.ceil - halfY; if (vel.y > 0) vel.y = 0; }   // the roof is a ceiling
      }
