(function (root, factory) {
  ...
  function zoneSchedule(rnd, bound) {
    var Z = ZONE, n = Z.shrinkPhases, circles = [{ cx: 0, cz: 0, r: Z.r0 }];
    var fa = rnd() * Math.PI * 2, fd = Math.sqrt(rnd()) * Z.finalCenterMax;
    var fx = Math.cos(fa) * fd, fz = Math.sin(fa) * fd;
    var lim = (bound || 120) - Z.boundPad;
    for (var k = 1; k <= n; k++) {
      var prev = circles[k - 1];
      var r = Z.r0 * Math.pow(Z.rFinal / Z.r0, k / n);
      var t = k / n;
      var cx = fx * t + (rnd() - 0.5) * 2 * (prev.r - r) * 0.35;
      var cz = fz * t + (rnd() - 0.5) * 2 * (prev.r - r) * 0.35;
      ...
      for (var it = 0; it < 40; it++) {
        ...
        if (r < lim) { cx = Math.max(-(lim - r), Math.min(lim - r, cx)); cz = Math.max(-(lim - r), Math.min(lim - r, cz)); }
      }
