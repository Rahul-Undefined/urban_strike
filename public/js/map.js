...
var World = (function () {
  ...
  // ---------- geometry helpers ----------
  function addCollider(x0, y0, z0, x1, y1, z1) { colliders.push([x0, y0, z0, x1, y1, z1]); }
  ...
  function lighting() {
    ...
    scene.fog = new THREE.FogExp2(0x1a1f28, 0.0085);

    var hemi = new THREE.HemisphereLight(0x93a7bd, 0x201a12, 0.5);
    scene.add(hemi);

    sun = new THREE.DirectionalLight(0xffab60, 0.95);
    sun.position.set(58, 62, 26);
    sun.castShadow = true;
    ...
    // Interior lights
    var wh = new THREE.PointLight(0xffb35c, 0.9, 32, 1.6); wh.position.set(-32, 6.8, -28); scene.add(wh);
    flickers.push(wh);
    var ap = new THREE.PointLight(0x8fb4ff, 0.55, 15, 1.8); ap.position.set(27, 4.6, -35); scene.add(ap);
    var st1 = new THREE.PointLight(0xffb35c, 0.8, 17, 1.7); st1.position.set(8.5, 5, -18); scene.add(st1);
    flickers.push(st1);
    var st2 = new THREE.PointLight(0xffb35c, 0.7, 17, 1.7); st2.position.set(-8.5, 5, 14); scene.add(st2);
  }

  function groundAndRoads() {
    // Ground slab (also the master floor collider)
    var g = box(0, -0.5, 0, 120, 1, 120, M.dirt, { cast: false });
    // Roads (visual planes on top of the ground)
    seg(-7, 7, 0.005, 0.02, -50, 50, M.asphalt, { collide: false, cast: false });
    seg(-50, 50, 0.005, 0.02, -7, 7, M.asphalt, { collide: false, cast: false });
    // Center dashes
    for (var z = -46; z <= 46; z += 3.4) { if (Math.abs(z) < 8.5) continue; seg(-0.1, 0.1, 0.021, 0.03, z, z + 1.5, M.white, { collide: false, cast: false }); }
    for (var x = -46; x <= 46; x += 3.4) { if (Math.abs(x) < 8.5) continue; seg(x, x + 1.5, 0.021, 0.03, -0.1, 0.1, M.white, { collide: false, cast: false }); }
    ...
    var curbsE = [
      [-46, -9, 7.2, 9.2], [9, 46, 7.2, 9.2], [-46, -9, -9.2, -7.2], [9, 46, -9.2, -7.2]
    ];
...
World.build = function (sceneRef) {
  ...
  /* ===== PERIMETER + SKYLINE ===== */
  seg(-50.9, 50.9, 0, 2.8, -50.9, -50, M.concrete);
  seg(-50.9, 50.9, 0, 2.8, 50, 50.9, M.concrete);
  seg(-50.9, -50, 0, 2.8, -50, 50, M.concrete);
  seg(50, 50.9, 0, 2.8, -50, 50, M.concrete);
  for (var i = 0; i < 10; i++) {
    var ang = (i / 10) * Math.PI * 2;
    var rr = 74 + rnd() * 18;
    box(Math.cos(ang) * rr, 6 + rnd() * 9, Math.sin(ang) * rr,
      8 + rnd() * 10, 12 + rnd() * 18, 8 + rnd() * 10, M.dark, { collide: false, cast: false, recv: false });
  }
