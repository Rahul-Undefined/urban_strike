# Urban Strike — Handoff (v10.8 shipped)

Upload this file plus `urban-strike-v10.8.zip` into a new chat.
...
- **PRESS F3.** It shows FPS and frame-time p50/p90/max. Six versions of "it
  is laggy" were diagnosed by guessing because nothing could report this.
  **Read p90, not the average** — a 16 ms mean hides a hitch every tenth frame.
- **Positional audio is capped at 70 m and uses `equalpower`, not `HRTF`.**
  Every sound used to build an HRTF panner at ANY distance; 19 bots walking =
  ~145 Web Audio nodes/sec for footsteps alone, most of them inaudible. That
  scaled with bot count and was the bot-mode stutter.
