...
// ---------------------------------------------------------------------------
/* ===== BASELINE RE-RECORDED FOR v9.7 — a decision, not a convenience =====
   Deliberate changes since v9.6, all signed off:
     - spawn points roughly doubled on every map (22->44, 24->46, 28->50).
       22 tiles for up to 20 humans PLUS 19 bots meant a Strike Team match had
       fewer places to stand than bodies.
     - ground loot raised (270->360, 156->246, 74->164).
       All generated by tools/gen-points.js from the built geometry, never typed.
     - Urban: the east bulkhead wall on the Civic apartment roof was SPLIT to
       open the way out of the stairwell. That is the only geometry edit, which
       is why `colliders` and `tris` below are unchanged from v9.6 and only the
       checksum moved.
   The v9.6 note it replaces recorded the South Terminal / Westbrook Stadium
   rebuild; see CHANGELOG.md for both.
   Recorded with: node tools/verify-untouched.js --record */
const BASELINE = {
  urban: {"colliders":3282,"sum":721674782,"draws":103,"tris":87396,"casters":57,"lights":7,"minimap":208,"bound":100},
  rural: {"colliders":1066,"sum":1837205283,"draws":32,"tris":54467,"casters":22,"lights":3,"minimap":210,"bound":150},
  urbanData: {"loot":360,"spawns":44,"airdrops":10},
