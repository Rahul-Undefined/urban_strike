...
console.log('--- the route wanders and the flight is endless ---');
{
  ...
  for (let seed = 1; seed <= 400; seed++) {
    ...
    /* v2.0: the route wanders the whole MAP — MAPS.urban.ext — not a +/-110 square */
    const EX = CFG.MAPS.urban.ext;
    for (const p of W) if (p[0] < EX.x0 || p[0] > EX.x1 || p[1] < EX.z0 || p[1] > EX.z1) onMap = false;
...
ok(Srv.start({ settings: { map: 'killhouse' } }) === null, 'not on an arena');
...
console.log('--- two machines, fuel, refuel (v1.0x) ---');
{
  ...
  /* v2.0: tick through the two minutes a second at a time (the real server
     ticks at 66 ms; one 120 s jump would fire the siren and the crash in the
     same tick and prove nothing about either) */
  const eb0 = emitted.length;
  for (let k = 0; k < H.secondSpawnSec; k++) { T0 += 1000; ride(); SrvX.tick(rx); }
  T0 += 100; ride(); SrvX.tick(rx);
  ok(rx.helis[1].state === 'pad', 'two minutes later machine B sits on pad B for the other side');
  /* ===== v2.0: FUEL ENDS IN A SIREN AND A CRASH, NOT A RETURN =====
     Rahul: "no auto return; if fuel is finished the helicopter will crash and
     the player will die." fuelWarnSec before empty the room gets heliFuel with
     the riders listed; at zero the state is 'crash' (CFG.heliCrashPose from
     the frozen pose), crashSec later the wreck is on the ground, every rider
     is dead (helidown) and the machine is gone until its respawn. */
  let steps = 0; while (rx.helis[0].state === 'flying' && steps++ < 200) { T0 += 1000; ride(); SrvX.tick(rx); }
  const flew = (T0 - rx.helis[0].flightStart) / 1000;
  ok(rx.helis[0].state === 'crash' && flew >= H.fuelSec && flew < H.fuelSec + 3, 'after ' + H.fuelSec + ' s of fuel the machine goes DOWN — no auto-return [' + flew.toFixed(0) + ' s]');
  const fe = emitted.slice(eb0).filter(e => e.ev === 'heliFuel').map(e => [e.ev, e.d]);
  ok(fe.length === 1 && fe[0][1].idx === 0 && fe[0][1].riders.indexOf('PX') >= 0 && fe[0][1].left <= H.fuelWarnSec && fe[0][1].left >= H.fuelWarnSec - 2,
    'the low-fuel siren fired once, ' + (fe.length ? fe[0][1].left : '?') + ' s before empty, naming the rider');
  const cp0 = CFG.heliCrashPose(cfgA, rx.helis[0].crashFrom, 0), cpEnd = CFG.heliCrashPose(cfgA, rx.helis[0].crashFrom, H.crashSec + 1);
  ok(cp0.phase === 'crash' && Math.abs(cp0.y - H.alt) < 0.5 && cpEnd.phase === 'wreck' && cpEnd.y === H.padY, 'the crash pose falls from altitude to the ground and stays there');
  ok(pil.alive && rx.helis[0].riders.indexOf('PX') >= 0, 'the rider is still listed (and alive) while the cabin falls — the floor carries them down');
  T0 += (H.crashSec + 0.5) * 1000; SrvX.tick(rx);
  ok(!pil.alive && killedX.indexOf('PX') >= 0, 'when the wreck meets the ground the rider is dead');
  ok(rx.helis[0].state === 'gone' && rx.helis[0].respawnAt > T0 + H.respawnSec * 1000 - 2000, 'the machine is gone until its respawn');
  const boom = emitted.slice(eb0).filter(e => e.ev === 'heliBoom').map(e => [e.ev, e.d]);
  ok(boom.length === 1 && boom[0][1].crash === 1 && boom[0][1].n === 1 && boom[0][1].by === null, 'heliBoom names the crash, one aboard, no killer');
  /* bring A back for the two-machine checks below */
  T0 = rx.helis[0].respawnAt + 50; SrvX.tick(rx);
  ok(rx.helis[0].state === 'pad', 'respawnSec later a fresh machine sits on pad A');
  pil.alive = true; pil.pos = [H.pads[0][0] + 3, half + H.padY, H.pads[0][1]];
  const rb = SrvX.board(rx, pil); pil.pos = rb.seat.slice();
  T0 += H.boardSec * 1000 + 100; SrvX.tick(rx);
  ok(rx.helis[0].state === 'flying', 'and Z aboard lifts it for another tank');
  const PX2 = SrvX.pathFor(rx.helis[0].seed, rx.helis[0].pad);
  const ride2 = () => { const q = CFG.heliPoseAt(cfgA, PX2, (T0 - rx.helis[0].t0) / 1000); pil.pos = [q.x, q.y + H.cabinFloor + half, q.z]; };
  ride2();
