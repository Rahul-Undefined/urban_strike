...
ok(S.T > 80 && S.T < 130, 'a lap with four dwells at ' + T.speed + ' m/s takes ' + S.T.toFixed(0) + ' s');   /* v2.1: ~2 km at 30 m/s */
{
  ...
  (T.stations || []).forEach(st => {
    const q = stopsWorld.find(q2 => (st.side === 'N' && Math.abs(q2.z - TRK.N) < 0.1) || (st.side === 'E' && Math.abs(q2.x - TRK.E) < 0.1) || (st.side === 'S' && Math.abs(q2.z - TRK.S) < 0.1) || (st.side === 'W' && Math.abs(q2.x - TRK.W) < 0.1));
    const along = (st.side === 'N' || st.side === 'S') ? q.x : q.z;   // head position along the platform axis
    const dir = (st.side === 'N' || st.side === 'E') ? 1 : -1;       // travel direction along that axis (clockwise loop)
    const nose = along, tailA = along - dir * (11 + 0.9 + 3 * 12 + 2 * 0.9);
    const lo = Math.min(nose, tailA), hi = Math.max(nose, tailA);
    ok(st.c - 26 <= lo + 1.5 && st.c + 26 >= hi - 1.5, st.name + ' deck (' + (st.c - 26) + '..' + (st.c + 26) + ') spans the whole train at the stop (' + lo.toFixed(0) + '..' + hi.toFixed(0) + ')');
  });
...
const hits = new Map();
for (const PP of [P]) for (let s = 0; s < PP.length; s += 2.0) {
  for (const o of offs) {
...
const tCruise = T.dwellSec + S.legs[0].tA + 2;   // v2.0: two seconds into the cruise — mid-straight at 30 m/s, before the corner
...
ok(carried > 0.5 * T.speed * 0.7 && carried < 0.5 * T.speed * 1.1, 'half a second on board carried the rider ' + carried.toFixed(2) + ' m with the train (' + (0.5 * T.speed).toFixed(1) + ' expected)');   /* v2.0: against the configured speed */
// walking sideways into the wall is held inside
/* v2.0: at 30 m/s the coach moved 15 m during the carry above — re-read where
   it stands NOW before placing anyone on it (at 10 m/s the stale pose was
   still inside the 12 m coach; it is not any more) */
cars = vm.runInContext('Train.cars().map(function(c){ return { x:c.x, z:c.z, yaw:c.yaw, vx:c.vx, vz:c.vz, coach:c.coach }; })', ctx);
coach = cars[1];
...
console.log('--- v1.0f: corners, walls, and what the train does to a bystander ---');
{
  ...
  /* v2.0: at every stop the +lz (city) side of the coach is now the HALT's
     1.05 m deck, so the ROAD test uses the outer (-lz) face and the BOARDING
     test uses the deck, which is how the halts are meant to be used. The
     wall invariant is "never inside the body" (|lz| >= HALF_W + 0.05) — the
     controller pushes out only once a player is inside the body, so the exact
     resting distance depends on the frame phase and is not the contract. */
  const outX = cc.x + Math.sin(cc.yaw) * (HALF_W + 0.9), outZ = cc.z - Math.cos(cc.yaw) * (HALF_W + 0.9);   // 0.9 m outside the -lz face, mid-coach
  vm.runInContext('PlayerCtl.spawnAt([' + outX + ', ' + (CFG.PLAYER.standH / 2 + 0.02) + ', ' + outZ + '], ' + (cc.yaw + Math.PI) + ');', ctx);   // yaw = heading + PI walks toward +lz, INTO the coach side
  ...
  let minLzW = 9;
  for (let f = 0; f < 120; f++) { poseAt(tStop); vm.runInContext('__run(1, __inpF)', ctx); const rq = vm.runInContext('({x: PlayerCtl.pos.x, z: PlayerCtl.pos.z})', ctx); minLzW = Math.min(minLzW, Math.abs(-(rq.x - cc.x) * Math.sin(cc.yaw) + (rq.z - cc.z) * Math.cos(cc.yaw))); }
  const rW = vm.runInContext('({x: PlayerCtl.pos.x, z: PlayerCtl.pos.z, y: PlayerCtl.pos.y, onPlat: PlayerCtl.onPlatform})', ctx);
  const lzW = -(rW.x - cc.x) * Math.sin(cc.yaw) + (rW.z - cc.z) * Math.cos(cc.yaw);
  ok(Math.abs(lzW) >= HALF_W + 0.05 && minLzW >= HALF_W - 0.1 && rW.y < 1.5 && !rW.onPlat, 'two seconds of walking into the coach side from the road leaves the player outside it, on the road [lateral ' + lzW.toFixed(2) + ' m, nearest ' + minLzW.toFixed(2) + ']');
  /* ...and the same walk from the DECK at a DOOR climbs aboard. */
  const doorLx = cc.L / 2 - 1.7, DECK = 1.05;
  const dX = cc.x + Math.cos(cc.yaw) * doorLx - Math.sin(cc.yaw) * (HALF_W + 1.2), dZ = cc.z + Math.sin(cc.yaw) * doorLx + Math.cos(cc.yaw) * (HALF_W + 1.2);   // on the deck, 1.2 m off the +lz face
  vm.runInContext('PlayerCtl.spawnAt([' + dX + ', ' + (DECK + CFG.PLAYER.standH / 2 + 0.02) + ', ' + dZ + '], ' + cc.yaw + ');', ctx);   // yaw = heading walks toward -lz, through the door
  for (let f = 0; f < 30; f++) { poseAt(tStop); vm.runInContext('__run(1, __inpF)', ctx); }   // ~2.5 m: off the deck, over the step, into the aisle
  const rD = vm.runInContext('({x: PlayerCtl.pos.x, z: PlayerCtl.pos.z, y: PlayerCtl.pos.y, onPlat: PlayerCtl.onPlatform})', ctx);
  ok(rD.onPlat && Math.abs(rD.y - (T.floor + CFG.PLAYER.standH / 2)) < 0.1, 'the same walk from the halt deck at a door boards the coach [y ' + rD.y.toFixed(2) + ']');
  /* and mid-coach from the deck: the wall holds you on the deck */
  const wX = cc.x - Math.sin(cc.yaw) * (HALF_W + 1.2), wZ = cc.z + Math.cos(cc.yaw) * (HALF_W + 1.2);
  vm.runInContext('PlayerCtl.spawnAt([' + wX + ', ' + (DECK + CFG.PLAYER.standH / 2 + 0.02) + ', ' + wZ + '], ' + cc.yaw + ');', ctx);
  for (let f = 0; f < 60; f++) { poseAt(tStop); vm.runInContext('__run(1, __inpF)', ctx); if (process.env.TRAIN_DBG && f % 5 === 0) console.log('    f', f, vm.runInContext('JSON.stringify({x:+PlayerCtl.pos.x.toFixed(2),y:+PlayerCtl.pos.y.toFixed(2),z:+PlayerCtl.pos.z.toFixed(2),g:PlayerCtl.grounded,p:PlayerCtl.onPlatform})', ctx)); }
  const rK = vm.runInContext('({x: PlayerCtl.pos.x, z: PlayerCtl.pos.z, y: PlayerCtl.pos.y, onPlat: PlayerCtl.onPlatform})', ctx);
  const lzK = -(rK.x - cc.x) * Math.sin(cc.yaw) + (rK.z - cc.z) * Math.cos(cc.yaw);
  ok(Math.abs(lzK) >= HALF_W + 0.05 && rK.y > DECK + 0.5 && !rK.onPlat, 'a second of walking into the coach wall from the deck leaves the player on the deck, outside [lateral ' + lzK.toFixed(2) + ', y ' + rK.y.toFixed(2) + ']');
  ...
  /* v1.0g: leaving a MOVING train is fatal; leaving at the stop is not. */
  {
    ...
    ok(/h\.v > 2\.0 && q\.trainAboardAt && t - q\.trainAboardAt < 1200 && feet < 0\.45/.test(hz), 'the rule is server-side: seen aboard within 1.2 s, train over 2 m/s, now at ground level');   /* v1.1.1 removed the toast; the rule stays */
  }
}
