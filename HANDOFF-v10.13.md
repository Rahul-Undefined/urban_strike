# Urban Strike — Handoff (v10.13 shipped)

Upload this file plus `urban-strike-v10.13.zip` into a new chat.
...
> (expect 36) and `curl -s <host>/version`. v10.13.0 verified on both.
...
1. **OUTBREAK — the whole mode.** Never played. Solo is the fastest check and
   it is the only mode on this build one person can start. Watch for: does
   wave 1 feel too easy and wave 10 too hard; do the dead read as frightening
   or as green soldiers; does the melee connect at a fair range; does the
   10 s breather feel right. Every one of those is a named constant at the top
   of `server/lib/zombies.js` — tell me which way and it is a one-line change.
   **No dedicated outbreak map yet** — it runs on urban, rural and metro.
2. **THE ENEMY SPOT (U).** Team modes only. Confirm it refuses through walls,
...
- **`CFG.MODES[m].outbreak` carries the zombie rules.** `lives: 1` reuses Last
...
- **Never let a zombie outrun a sprint.** `waveSpeed` asymptotes at 0.96x on
  purpose; a RUNNER multiplies it by 1.45 and is still under sprint. Gated.
...
## 11. KILLHOUSE — shipped v10.10, extended v10.11
...
Indoor warehouse, 58 x 34 m, humans only, mirrored about x=0 by `pair()`.
    ...
    public/src/environment/killhouse.js            geometry, segx(), KEEP_CLEAR
    server/lib/nuke.js                             the killstreak
    CFG.MAPS.killhouse                             bound 32, maxPlayers 8

    draws 33/45 · tris 12,248/26,000 · casters 17/22 · colliders 204
    dead ground 0.2% (budget 0.02) · floaters 0 · escapes 0 · worst uncovered 14.7 m

**maxPlayers 8 on the MAP caps every mode played on it**, lower of map and mode
wins. **No sniper or RPG on the floor** — airdrop pool only. **No stairs**;
container tops come off a 0.31 m crate chain.

### The nuke (v10.11)

Five kills without dying arms it. `nukeReady` to that player alone. N opens the
full map in targeting mode; a click calls an 11 m strike for 10 s at 55 damage
per 500 ms. Never the caller, never a teammate, re-checked every tick.

Tuning is five constants at the head of `server/lib/nuke.js`: REQ_STREAK,
RADIUS, DURATION, TICK_MS, TICK_DMG. **Change them there and nowhere else** —
the client reads radius and duration off the wire.

**Dying loses it.** That is the reward, not a limitation of it.

### Two things NOT to do to it

- **Do not add a second streak counter.** It rides combat.js's `streak`, which
  combat.js already resets on death.
- **Do not move the damage off applyDamage().** A nuke kill must use the same
  armour, kill feed, assist and win-condition path as a bullet.
...
## 14. OUTBREAK — shipped v10.13

    server/lib/zombies.js     waves, curve, melee, scoring
    CFG.MODES zsolo/zduo/zsqd/zfull     cat 'zomb', lives 1, outbreak: true
    Avatars.makeZombie()      the dead, applied over the ordinary rig
    tools/verify-outbreak.js  33 assertions

One life, waves, 10 s between, wave 100 is the end and a WIN. Solo to fifteen.

**A zombie is a bot.** bots.js is retained and switched off; this rides its
pathing, stairs, climbing, line of sight and spawning. The melee is the only
new behaviour, and bots.js had already named it as the gap.

### Tuning lives in five constants at the head of zombies.js
`MAX_WAVE 100 · COOLDOWN_MS 10000 · MAX_CONCURRENT 26 · MELEE_REACH 2.1 ·
MELEE_CD_MS 900`, plus the four curve functions. Change them THERE.

### Not done
- **No dedicated outbreak map.** Runs on urban, rural and metro, which have the
  range for every weapon class. Killhouse and Sunset Row are excluded: 8-player
  rooms with sightlines under 40 m cannot host "sniper, assault and all other
  guns can function properly".
- **No perks, no buy stations, no pack-a-punch.** The reference games are built
  on an economy. Scoring exists and is broadcast; nothing spends it yet. That
  is the obvious next layer and it should be designed after the base loop has
  actually been played.
- **No zombie audio.** Same reason as the menu music: there is no audio asset
  pipeline, and a wave of ninety needs a pooled, distance-capped sound budget
  designed deliberately — see the v10.7 lesson about bot gunfire being the lag.
